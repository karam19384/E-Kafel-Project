...
class EditOrphanDetailsScreen extends StatefulWidget {
    static const routeName = '/edit_orphan_details_screen';

  final String orphanId;
  final String institutionId;
  final Orphan orphanData;

  const EditOrphanDetailsScreen({
    super.key,
    required this.orphanId,
    required this.institutionId, 
    required  this.orphanData,
...
class _EditOrphanDetailsScreenState extends State<EditOrphanDetailsScreen> {
  final List<String> _grades = ['لا يدرس','روضة','الصف الأول','الصف الثاني','الصف الثالث','الصف الرابع','الصف الخامس','الصف السادس','الصف السابع','الصف الثامن','الصف التاسع','الصف العاشر','الصف الحادي عشر','الصف الثاني عشر'];
  final List<String> _healthOptions = ['سليم','سكري','ربو','فقر دم','حساسية','أمراض قلب','إعاقة سمعية','إعاقة بصرية','إعاقة حركية','أخرى'];
  String? _selectedGrade; String? _selectedHealth; String? _selectedOrphanType;
  final TextEditingController _orphanFirstController = TextEditingController();
  final TextEditingController _orphanFatherController = TextEditingController();
  final TextEditingController _orphanGrandController = TextEditingController();
  final TextEditingController _orphanGreatGrandController = TextEditingController();
  final TextEditingController _orphanFamilyController = TextEditingController();
  ...
  void initState() {
  super.initState();
  _orphanFirstController.text = widget.orphanData.orphanName;
  _orphanFatherController.text = widget.orphanData.fatherName;
  _orphanGrandController.text = widget.orphanData.grandfatherName;
  _orphanGreatGrandController.text = widget.orphanData.greatGrandfatherName;
  _orphanFamilyController.text = widget.orphanData.familyName;
  _selectedOrphanType = widget.orphanData.orphanType;
  _selectedGrade = widget.orphanData.schoolName;
  _selectedHealth = widget.orphanData.healthStatus;
}
  ...
  void _saveChanges() {
    if (_formKey.currentState!.validate()) {
      ...
      context.read<OrphansBloc>().add(
            UpdateOrphan( orphanId: widget.orphanId, institutionId: widget.institutionId, updatedData: updatedData),
          );
    }
  }
